...
public class BonusState : MonoBehaviour
{
    ...
    private int selectedStarIndex = -1; // ????? ???? ?¥å???
    private int totalStars = 4; // ???? ?? ???? (???¡À? 5?? ????)
    ...
    {
        StatText.text = "³²Àº º¸³Ê½º ½ºÅÈ = " + PlayerPrefs.GetInt("RemainStat", bonusstat);
    }
